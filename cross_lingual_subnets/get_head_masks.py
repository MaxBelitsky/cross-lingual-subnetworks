...
def compute_heads_importance(
    ...
    # preparing tensors
    head_importance, attn_entropy, head_mask = init_tensors(
        model, args.device, head_mask, compute_importance
    )
    ...
    for step, batch in enumerate(
        ...
        if compute_entropy:
            for layer, attn in enumerate(all_attentions):
                masked_entropy = entropy(attn.detach()) * input_mask.float().unsqueeze(
                    1
                )
    ...
    if compute_entropy:
        ...
        np.save(
            os.path.join(args.output_dir, save_prefix + "attn_entropy.npy"),
    ...
    if compute_importance:
        ...
        # Print/save matrices
        np.save(
            os.path.join(args.output_dir, save_prefix + "head_importance.npy"),
...
def compute_metrics(task_name, preds, labels):
    ...
    if task_name == "mlm":
        return {
            "perplexity": Perplexity().compute(predictions=preds, references=labels)
        }


def mask_heads(args, model, eval_dataloader):
    """
    Shows how to mask head (set some heads to zero), to test the effect on the network,
    based on the head importance scores, as described in Michel et al.
    http://arxiv.org/abs/1905.10650
    """
    save_prefix = args.language + "_seed_" + str(args.seed) + "_"

    _, head_importance, perplexity = compute_heads_importance(
        args, model, eval_dataloader, compute_entropy=False
    )
    original_score = perplexity
    logger.info(
        "Pruning: original score (perplexity): %f, threshold: %f",
        original_score,
    ...
    while current_score <= original_score * args.masking_threshold:
        ...
        if args.save_mask_all_iterations:
            np.save(
                os.path.join(args.output_dir, f"head_mask_{i}.npy"),
                head_mask.detach().cpu().numpy(),
            )
            np.save(
                os.path.join(args.output_dir, f"head_importance_{i}.npy"),
                head_importance.detach().cpu().numpy(),
            )
        i += 1
    ...
    np.save(
        os.path.join(args.output_dir, save_prefix + "head_mask.npy"),
        head_mask.detach().cpu().numpy(),
    )
...
def prune_heads(args, model, eval_dataloader, head_mask):
    ...
    the head importance scores as described in Michel et al.
    (http://arxiv.org/abs/1905.10650)
    """
...
def main():
    ...
    parser.add_argument(
        "--metric_name", default=None, type=str, help="Metric to use for head masking."
    )
    ...
    if args.server_ip and args.server_port:
        # Distant debugging - see https://code.visualstudio.com/docs/python/debugging#_attach-to-a-local-script # noqa
        import ptvsd
    ...
    if args.metric_name is None:
        args.metric_name = {"mnli": "acc", "mlm": "perplexity"}[args.task_name]

    # Load pretrained model and tokenizer
    if args.local_rank not in [-1, 0]:
        # Make sure only the first process in distributed training will download
        # model & vocab
        torch.distributed.barrier()

    assert (
        args.model_name is None or args.model_path is None
    ), "Only one of model_name and model_path should be used"
